...
endp checkForHit
; ----------------------------------------------------
proc checkForHitNewShot
; this procedure is checking if the bullet had hit an alien. if so, it deletes the alien and the bullet, and puts
; 0 in the alien's position in the array, meaning killing it. if not, procedure moves on.
; note: this procedure is different from the previous one- it is checking for the hit
;       when a new shot is fired. this is necessary because the aliens might pass the bullet's height,
;       but not the spaceship's.
; on entry: Alien's height
;			Alien's width
;			aliens' x array reference
;			aliens' Y array reference
...
checkAlienYNewShot:
	mov ax,[word ptr bx+si]
	add ax,[bp+12] ;check if the bullet is under the alien (add it's height to check under and not on)
	ja checkAlienXNewShot ;bullet is passed the alien's height
	cmp ax,[bulletY]
	ja checkAlienXNewShot ;if the bullet is higher than the bottom of the alien, but lower than it's top, its a hit
YnotHitNewShot:
	add si,2
	loop checkAlienYNewShot
	jmp hitAppliedNewShot
...
maybeXhitNewShot:
	...
	add ax,[bp+10] ;alien's width
	cmp ax,[bulletX] ;check if the bullet's x is to the left of the right side of the alien
	jae bulletHitNewShot ;if it is, its a hit
	; if it isn't, loop again:
	add si,2 
	mov bx,[bp+6]
	loop checkAlienYNewShot
